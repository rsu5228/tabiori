...
if(!Array.isArray(tripList)||!tripList.length){
 tripList=[{id:"seoul-2026",title:"ソウル2泊3日",date:"9/5〜9/7",destination:"🇰🇷",hotel:"相鉄フレッサイン ソウル明洞"}];
...
function getTrip(id){return tripList.find(t=>String(t.id)===String(id));}
...
function openTrip(id){
 ...
 currentTripId=t.id; localStorage.setItem("tabiori_current_trip",t.id);
 document.getElementById("tripListView")?.classList.add("hidden");
 document.getElementById("tripDetailView")?.classList.remove("hidden");
 try{selectedDay=0;if(typeof render==="function")render();}catch(e){console.warn(e);}
 document.getElementById("tripListView")?.classList.add("hidden");
 document.getElementById("tripDetailView")?.classList.remove("hidden");
}
function renderTripList(){
 const box=document.getElementById("tripCards"); if(!box)return;
 box.innerHTML=tripList.map(t=>`<div class="trip-card-wrap"><button type="button" class="trip-card" data-trip-id="${esc(t.id)}"><div class="trip-card-main"><div class="trip-card-title">${esc(t.destination||"")} ${esc(t.title||"")}</div><div class="trip-card-date">${esc(t.date||"")}</div>${t.hotel?`<div class="trip-card-hotel">${esc(t.hotel)}</div>`:""}</div><span class="trip-card-arrow">›</span></button></div>`).join("");
}
function addTrip(){
 const id="trip-"+Date.now(); tripList.push({id,title:"新しい旅行",date:"",destination:"🇯🇵",hotel:""});
 localStorage.setItem("tabiori_trips",JSON.stringify(tripList)); renderTripList(); openTrip(id); setTimeout(()=>openTripEditor(id),0);
}
function openTripEditor(id){
 const t=getTrip(id),b=document.getElementById("tripEditBackdrop"); if(!t||!b)return;
 document.getElementById("tripEditTitle").value=t.title||"";
 document.getElementById("tripEditDestination").value=t.destination||"";
 let s=t.startDate||"",e=t.endDate||""; if((!s||!e)&&t.date){let p=String(t.date).split("〜");if(p.length===2){s=p[0].trim();e=p[1].trim();}}
 document.getElementById("tripEditStartDate").value=s; document.getElementById("tripEditEndDate").value=e; document.getElementById("tripEditHotel").value=t.hotel||"";
 b.dataset.tripId=t.id;b.classList.remove("hidden");
}
function closeTripEditor(){document.getElementById("tripEditBackdrop")?.classList.add("hidden");}
function saveTripEditor(){
 const b=document.getElementById("tripEditBackdrop"),t=getTrip(b?.dataset.tripId);if(!t)return;
 t.title=document.getElementById("tripEditTitle")?.value.trim()||"無題の旅行";
 t.destination=document.getElementById("tripEditDestination")?.value.trim()||"";
 t.startDate=document.getElementById("tripEditStartDate")?.value||"";t.endDate=document.getElementById("tripEditEndDate")?.value||"";
 t.date=t.startDate&&t.endDate?`${t.startDate}〜${t.endDate}`:(t.startDate||t.endDate);
 t.hotel=document.getElementById("tripEditHotel")?.value.trim()||"";
 localStorage.setItem("tabiori_trips",JSON.stringify(tripList));closeTripEditor();showTripList();
}
function deleteTripEditor(){
 const b=document.getElementById("tripEditBackdrop"),id=b?.dataset.tripId;if(!id)return;
 if(tripList.length<=1){alert("最後の旅行は削除できません。");return;}
 const t=getTrip(id);if(!t)return;if(!confirm(`「${t.title||"この旅行"}」を削除しますか？`))return;
 tripList=tripList.filter(x=>String(x.id)!==String(id));localStorage.setItem("tabiori_trips",JSON.stringify(tripList));
 if(String(currentTripId)===String(id)){currentTripId=tripList[0].id;localStorage.setItem("tabiori_current_trip",currentTripId);}
 closeTripEditor();showTripList();
}
function bindTripManagerV074(){
 const cards=document.getElementById("tripCards");
 if(cards&&!cards.dataset.bound074){
  cards.dataset.bound074="1";let timer=null;
  cards.addEventListener("click",e=>{let c=e.target.closest(".trip-card");if(c)openTrip(c.dataset.tripId);});
  cards.addEventListener("pointerdown",e=>{let c=e.target.closest(".trip-card");if(!c)return;clearTimeout(timer);timer=setTimeout(()=>openTripEditor(c.dataset.tripId),650);});
  ["pointerup","pointercancel","pointerleave"].forEach(x=>cards.addEventListener(x,()=>clearTimeout(timer)));
  cards.addEventListener("contextmenu",e=>{let c=e.target.closest(".trip-card");if(!c)return;e.preventDefault();openTripEditor(c.dataset.tripId);});
 }
 const add=document.getElementById("addTripButton");if(add&&!add.dataset.bound074){add.dataset.bound074="1";add.addEventListener("click",addTrip);}
 const back=document.getElementById("backToTrips");if(back&&!back.dataset.bound074){back.dataset.bound074="1";back.addEventListener("click",showTripList);}
 const close=document.getElementById("closeTripEditor");if(close&&!close.dataset.bound074){close.dataset.bound074="1";close.addEventListener("click",closeTripEditor);}
 const save=document.getElementById("saveTripEditor");if(save&&!save.dataset.bound074){save.dataset.bound074="1";save.addEventListener("click",saveTripEditor);}
 const del=document.getElementById("deleteTripEditor");if(del&&!del.dataset.bound074){del.dataset.bound074="1";del.addEventListener("click",deleteTripEditor);}
 const b=document.getElementById("tripEditBackdrop");if(b&&!b.dataset.bound074){b.dataset.bound074="1";b.addEventListener("click",e=>{if(e.target===b)closeTripEditor();});}
}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>{bindTripManagerV074();showTripList();},{once:true});else{bindTripManagerV074();showTripList();}
