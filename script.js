...
/* ===== v0.8.0 startup screen controller ===== */
(function(){
  const KEY="tabiori_trips";
  let trips=[];
  try{trips=JSON.parse(localStorage.getItem(KEY)||"[]");}catch(e){}
  ...
  function showList(){
    document.body.classList.remove("tabiori-trip-open");
    const list=$("#tripListView"), detail=$("#tripDetailView");
    if(list)list.classList.remove("hidden");
    if(detail){detail.classList.remove("tabiori-open");detail.classList.add("hidden")}
    renderList();
  }
  ...
  function openTrip(id){
    ...
    document.body.classList.add("tabiori-trip-open");
    const list=$("#tripListView"), detail=$("#tripDetailView");
    if(list)list.classList.add("hidden");
    if(detail){detail.classList.remove("hidden");detail.classList.add("tabiori-open")}
    try{
  ...
  function bind(){
    ...
    if(list&&!list.dataset.v080){
      list.dataset.v080="1";
      list.addEventListener("click",function(e){
        const card=e.target.closest("[data-trip-open]");
        if(!card)return;
        e.preventDefault();
        openTrip(card.dataset.tripOpen);
      });
    }
    const back=$("#backToTrips");
    if(back&&!back.dataset.v080){
      back.dataset.v080="1";
  ...
  function boot(){
    // Critical: startup state is ALWAYS the trip list.
    document.body.classList.remove("tabiori-trip-open");
    const list=$("#tripListView"), detail=$("#tripDetailView");
    if(list)list.classList.remove("hidden");
    if(detail){detail.classList.remove("tabiori-open");detail.classList.add("hidden")}
