...
function openTrip(id) {
  ...
  // Do not rely on an event listener attached to dynamically replaced cards.
  // render() is only for the itinerary itself.
  render();
  ...
  box.innerHTML = tripList.map(trip => `
    <div class="trip-card-wrap">
      <button type="button"
        ...
        onclick="openTrip(this.dataset.tripId)">
...
function openTripEditor(id) {
  ...
  // Support both the old combined date and the new start/end values.
  ...
  document.getElementById("tripEditStartDate").value = start;
  document.getElementById("tripEditEndDate").value = end;
...
function saveTripEditor() {
  ...
  const title = document.getElementById("tripEditTitle")?.value.trim() || "無題の旅行";
  const flag = document.getElementById("tripEditDestination")?.value.trim() || "";
  const start = document.getElementById("tripEditStartDate")?.value || "";
  const end = document.getElementById("tripEditEndDate")?.value || "";
  const hotel = document.getElementById("tripEditHotel")?.value.trim() || "";

  trip.title = title;
  trip.destination = flag;
  trip.startDate = start;
  trip.endDate = end;
  trip.date = start && end ? `${start}〜${end}` : (start || end);
  trip.hotel = hotel;
...
function bindTripManagerV073() {
  const addButton = document.getElementById("addTripButton");
  if (addButton && !addButton.dataset.bound073) {
    addButton.dataset.bound073 = "1";
    addButton.addEventListener("click", addTrip);
  }

  const backButton = document.getElementById("backToTrips");
  if (backButton && !backButton.dataset.bound073) {
    backButton.dataset.bound073 = "1";
    backButton.addEventListener("click", showTripList);
  }

  // Long press is attached once to the stable tripCards container.
  const cards = document.getElementById("tripCards");
  if (cards && !cards.dataset.bound073) {
    cards.dataset.bound073 = "1";
    let timer = null;
    ...
    cards.addEventListener("pointerdown", e => {
      ...
      timer = setTimeout(() => {
        longPressed = true;
        openTripEditor(card.dataset.tripId);
      }, 650);
    });

    ["pointerup", "pointercancel", "pointerleave"].forEach(type => {
      cards.addEventListener(type, () => clearTimeout(timer));
    });
