...
async function fetchWeather(city,date){const rawCity=(city||'').trim();const rawDate=(date||'').trim();if(!rawCity||!rawDate)return null;const cacheKey='weather:'+rawCity.toLowerCase()+':'+rawDate;if(weatherCache[cacheKey])return weatherCache[cacheKey];const geo=await getWeatherCoords(rawCity);const todayKey=localDateKey();const targetMs=Date.parse(rawDate+'T00:00:00');const todayMs=Date.parse(todayKey+'T00:00:00');if(!Number.isFinite(targetMs)||!Number.isFinite(todayMs))return null;const daily='weather_code,temperature_2m_max,temperature_2m_min';let j=null,kind='';if(rawDate<todayKey){const historicalForecastUrl=`https://historical-forecast-api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&start_date=${rawDate}&end_date=${rawDate}&daily=${daily}&timezone=${encodeURIComponent(geo.timezone)}`;try{const r=await fetch(historicalForecastUrl);if(r.ok){const candidate=await r.json();if(candidate?.daily?.time?.includes(rawDate))j=candidate;}}catch(e){}if(!j){const archiveUrl=`https://archive-api.open-meteo.com/v1/archive?latitude=${geo.lat}&longitude=${geo.lon}&start_date=${rawDate}&end_date=${rawDate}&daily=${daily}&timezone=${encodeURIComponent(geo.timezone)}`;try{const r=await fetch(archiveUrl);if(r.ok){const candidate=await r.json();if(candidate?.daily?.time?.includes(rawDate))j=candidate;}}catch(e){}}if(!j)return null;kind='actual';}else{const diff=Math.round((targetMs-todayMs)/86400000);if(diff>16)return null;const url=`https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&daily=${daily}&forecast_days=16&timezone=${encodeURIComponent(geo.timezone)}`;const r=await fetch(url);if(!r.ok)throw new Error('weather');j=await r.json();kind='forecast';}const i=j.daily?.time?.indexOf(rawDate);if(i==null||i<0)return null;const out={city:geo.name||rawCity,date:rawDate,kind,code:j.daily.weather_code?.[i],max:j.daily.temperature_2m_max?.[i],min:j.daily.temperature_2m_min?.[i]};if(out.code==null&&out.max==null&&out.min==null)return null;weatherCache[cacheKey]=out;saveWeatherCache();return out;}
async function renderWeather(t,dayIndex){const el=$('weatherWidget');if(!el)return;const iconEl=el.querySelector('.weather-icon');const textEl=el.querySelector('.weather-text');const endIconEl=el.querySelector('.weather-end-icon');const city=(t.weatherCity||'').trim();const date=t.startDate?formatDateOnly(t.startDate,dayIndex):'';el.classList.remove('hidden');el.disabled=false;iconEl.textContent='';endIconEl.textContent='';textEl.textContent='';if(!city||!date){iconEl.textContent='☀️';textEl.textContent='天気不明';endIconEl.textContent='☔️';return;}textEl.textContent='天気を読み込み中…';try{const w=await fetchWeather(city,date);if(!w){iconEl.textContent='☀️';textEl.textContent='天気不明';endIconEl.textContent='☔️';return;}const [icon]=weatherLabel(w.code);iconEl.textContent=icon;const max=Number.isFinite(Number(w.max))?`${Math.round(Number(w.max))}℃`:'--℃';const min=Number.isFinite(Number(w.min))?`${Math.round(Number(w.min))}℃`:'--℃';textEl.textContent=`${w.city} ${max}/${min}`;}catch(e){iconEl.textContent='☀️';textEl.textContent='天気不明';endIconEl.textContent='☔️';}}
function render(){
