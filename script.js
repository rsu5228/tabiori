...
saveItineraries();
let currentTripId=null,selectedDay=0,detailIndex=null,timeIndex=null,editingTripId=null,longPressTriggered=false,addPlanIcon='➕️',detailEditIcon='';
...
function openTripEdit(id){const t=tripList.find(x=>x.id===id);if(!t)return;editingTripId=id;$('tripEditTitle').value=t.title||'';$('tripEditFlag').value=t.flag||'';$('tripEditStart').value=t.startDate||'';$('tripEditEnd').value=t.endDate||'';$('tripEditWeatherCity').value=t.weatherCity||'';$('tripEditHotel').value=t.hotel||'';show('tripEditBackdrop');lock();}
...
function bind(){
 document.body.addEventListener('click',e=>{
   ...
   if(e.target.closest('#closeTripEdit')){closeAllModals();return;}if(e.target.closest('#saveTrip')){const t=tripList.find(x=>x.id===editingTripId);if(!t)return;t.title=$('tripEditTitle').value.trim()||'無題の旅行';t.flag=$('tripEditFlag').value.trim();t.startDate=$('tripEditStart').value;t.endDate=$('tripEditEnd').value;t.weatherCity=$('tripEditWeatherCity').value.trim();t.hotel=$('tripEditHotel').value.trim();saveAll();closeAllModals();renderTripList();if(currentTripId===t.id)render();return;}if(e.target.closest('#deleteTrip')){const idx=tripList.findIndex(x=>x.id===editingTripId);if(idx<0)return;const id=editingTripId;tripList.splice(idx,1);delete itineraries[id];saveAll();closeAllModals();showList();return;}
 });
