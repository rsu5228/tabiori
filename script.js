...
async function renderWeather(t,dayIndex){const el=$('weatherWidget');if(!el)return;const iconEl=el.querySelector('.weather-icon');const textEl=el.querySelector('.weather-text');const city=(t.weatherCity||'').trim();const date=t.startDate?formatDateOnly(t.startDate,dayIndex):'';el.classList.remove('hidden');el.disabled=false;iconEl.textContent='';if(!city||!date){textEl.textContent='☀天気不明☔';return;}textEl.textContent='天気を読み込み中…';try{const w=await fetchWeather(city,date);if(!w){textEl.textContent='☀天気不明☔';return;}const [icon]=weatherLabel(w.code);const max=Number.isFinite(Number(w.max))?`${Math.round(Number(w.max))}℃`:'--℃';const min=Number.isFinite(Number(w.min))?`${Math.round(Number(w.min))}℃`:'--℃';textEl.textContent=`${w.city} ${icon} ${max}/${min}`;}catch(e){textEl.textContent='☀天気不明☔';}}
