...
async function renderWeather(t,dayIndex){const el=$('weatherWidget');if(!el)return;const iconEl=el.querySelector('.weather-icon');const textEl=el.querySelector('.weather-text');const city=(t.weatherCity||'').trim();const date=t.startDate?formatDateOnly(t.startDate,dayIndex):'';el.classList.remove('hidden');el.disabled=false;if(!city||!date){iconEl.textContent='';textEl.textContent='都市を設定';return;}iconEl.textContent='';textEl.textContent='天気を読み込み中…';try{const w=await fetchWeather(city,date);if(!w){iconEl.textContent='';textEl.textContent='予報はまだ未発表';return;}const [icon]=weatherLabel(w.code);const max=Number.isFinite(Number(w.max))?`${Math.round(Number(w.max))}℃`:'--℃';const min=Number.isFinite(Number(w.min))?`${Math.round(Number(w.min))}℃`:'--℃';iconEl.textContent='';textEl.textContent=`${w.city} ${icon} ${max}/${min}`;}catch(e){iconEl.textContent='';textEl.textContent='天気を取得できません';}}
