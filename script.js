...
const WEATHER_CACHE_KEY='tabiori_weather_cache_v2';
...
async function fetchWeather(city,date){
  ...
  if(rawDate<todayKey){
    ...
    url=`https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&daily=weather_code,temperature_2m_max,temperature_2m_min&forecast_days=16&timezone=${encodeURIComponent(geo.timezone||'auto')}`;
    kind='forecast';
  }
  const r=await fetch(url);
  if(!r.ok)throw new Error('weather HTTP '+r.status);
  const j=await r.json();
  const i=j.daily?.time?.indexOf(rawDate);
