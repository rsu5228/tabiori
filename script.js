...
function normalize(days){return days.map(d=>({title:d.title,items:(d.items||[]).map(x=>({time:x.time||'09:00',icon:x.icon||'📍',name:x.name||'予定',detail:x.detail||'' ,skipped:!!x.skipped}))}));}
// 韓国旅行の予定が新しい保存領域に無い／空の場合は、旧データを復元し、無ければ初期予定を入れる。
if(!itineraries['seoul-2026'] || !itineraries['seoul-2026'].some(d=>Array.isArray(d?.items)&&d.items.length)){
  let restored=null;
  try{
    const legacy=JSON.parse(localStorage.getItem(OLD_KEY)||'null');
    if(Array.isArray(legacy) && legacy.some(d=>Array.isArray(d?.items)&&d.items.length)) restored=normalize(legacy);
  }catch(e){}
  itineraries['seoul-2026']=restored||clone(DEFAULT_ITINERARY);
}
Object.keys(itineraries).forEach(k=>{itineraries[k]=normalize(itineraries[k]);});
localStorage.setItem(ITIN_KEY,JSON.stringify(itineraries));
...
function saveAll(){localStorage.setItem(TRIPS_KEY,JSON.stringify(tripList));localStorage.setItem(ITIN_KEY,JSON.stringify(itineraries));}
function ensureItems(id){if(!itineraries[id])itineraries[id]=[{title:'DAY 1',items:[]},{title:'DAY 2',items:[]},{title:'DAY 3',items:[]}];}
...
function renderTripList(){const box=$('tripCards');box.innerHTML=tripList.map(t=>`<button type="button" class="trip-card" data-trip-id="${t.id}"><div class="trip-card-main"><div class="trip-card-title">${esc(t.flag||'✈️')} ${esc(t.title||'無題の旅行')}</div><div class="trip-card-date">${esc(formatDate(t.startDate,t.endDate))}</div>${t.hotel?`<div class="trip-card-hotel">${esc(t.hotel)}</div>`:''}</div><span class="trip-card-arrow">›</span></button>`).join('');}
...
function openDetail(i){const x=itineraries[currentTripId][selectedDay].items[i];if(!x)return;detailIndex=i;$('detailEditTitle').value=x.name||'';$('detailEditLocation').value=x.location||'';$('detailEditMemo').value=x.detail||'';show('detailBackdrop');lock();}
function openTimeEdit(i){const x=itineraries[currentTripId][selectedDay].items[i];if(!x)return;timeIndex=i;$('timeEditName').textContent=x.name;$('timeEditInput').value=x.time;show('timeEditBackdrop');lock();}
function openAdd(){ $('addPlanName').value='';$('addPlanTime').value='';$('addPlanDetail').value='';show('addPlanBackdrop');lock();}
function openAdjust(){editingDays=clone(itineraries[currentTripId]);$('currentTime').textContent=new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit',hour12:false});renderAdjust();show('modalBackdrop');lock();}
...
function openTripEdit(id){const t=tripList.find(x=>x.id===id);if(!t)return;editingTripId=id;$('tripEditTitle').value=t.title||'';$('tripEditFlag').value=t.flag||'✈️';$('tripEditStart').value=t.startDate||'';$('tripEditEnd').value=t.endDate||'';$('tripEditHotel').value=t.hotel||'';show('tripEditBackdrop');lock();}
...
function bind(){
 document.body.addEventListener('click',e=>{
   ...
   if(e.target.closest('#closeDetail')){closeAllModals();return;} if(e.target.closest('#saveDetail')){const x=itineraries[currentTripId][selectedDay].items[detailIndex];x.name=$('detailEditTitle').value.trim()||'予定';x.location=$('detailEditLocation').value.trim();x.detail=$('detailEditMemo').value;delete x.tag;saveAll();closeAllModals();render();return;} if(e.target.closest('#deleteDetailPlan')){itineraries[currentTripId][selectedDay].items.splice(detailIndex,1);saveAll();closeAllModals();render();return;}
   if(e.target.closest('#closeTimeEdit')){closeAllModals();return;}if(e.target.closest('#saveTimeEdit')){const x=itineraries[currentTripId][selectedDay].items[timeIndex];x.time=$('timeEditInput').value||x.time;saveAll();closeAllModals();render();return;}
   if(e.target.closest('#addPlanButton')){openAdd();return;}if(e.target.closest('#closeAddPlan')||e.target.closest('#cancelAddPlan')){closeAllModals();return;}if(e.target.closest('#saveAddPlan')){const name=$('addPlanName').value.trim();if(!name)return alert('予定名を入力してください。');itineraries[currentTripId][selectedDay].items.push({time:$('addPlanTime').value||'09:00',icon:'📍',name,detail:$('addPlanDetail').value});itineraries[currentTripId][selectedDay].items.sort((a,b)=>toMin(a.time)-toMin(b.time));saveAll();closeAllModals();render();return;}
   if(e.target.closest('#refreshTime')){$('currentTime').textContent=new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit',hour12:false});return;}if(e.target.closest('#resetSchedule')){editingDays=clone(DEFAULT_ITINERARY);renderAdjust();return;}if(e.target.closest('#closeAdjust')){closeAllModals();return;}if(e.target.closest('#saveSchedule')){itineraries[currentTripId]=normalize(editingDays);saveAll();closeAllModals();render();return;}
   ...
   if(e.target.closest('#addTripButton')){const id=makeId();tripList.push({id,title:'新しい旅行',flag:'✈️',startDate:'',endDate:'',hotel:''});ensureItems(id);saveAll();renderTripList();openTripEdit(id);return;}
   if(e.target.closest('#closeTripEdit')){closeAllModals();return;}if(e.target.closest('#saveTrip')){const t=tripList.find(x=>x.id===editingTripId);if(!t)return;t.title=$('tripEditTitle').value.trim()||'無題の旅行';t.flag=$('tripEditFlag').value.trim()||'✈️';t.startDate=$('tripEditStart').value;t.endDate=$('tripEditEnd').value;t.hotel=$('tripEditHotel').value.trim();saveAll();closeAllModals();renderTripList();if(currentTripId===t.id)render();return;}if(e.target.closest('#deleteTrip')){const idx=tripList.findIndex(x=>x.id===editingTripId);if(idx<0)return;const id=editingTripId;tripList.splice(idx,1);delete itineraries[id];saveAll();closeAllModals();showList();return;}
 });
