...
function normalize(days){
  ...
  return days.map((d,i)=>({title:d?.title||`DAY ${i+1}`,items:Array.isArray(d?.items)?d.items.map(x=>({time:x?.time||'09:00',icon:x?.icon||'➕️',name:x?.name||'予定',detail:x?.detail||'',skipped:!!x?.skipped})):[]}));
}
