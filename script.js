...
tripList=tripList.map(t=>{
  ...
  if(!weatherCity && /シンガポール|Singapore/i.test(String(t.title||''))) weatherCity='Singapore';
...
const WEATHER_CACHE_KEY='tabiori_weather_cache_v3';
...
async function getWeatherCoords(city){
  const raw=(city||'').trim();
  const key='geo:'+raw.toLowerCase();
  if(!raw)throw new Error('city');
  if(weatherCache[key])return weatherCache[key];
  const u='https://geocoding-api.open-meteo.com/v1/search?name='+encodeURIComponent(raw)+'&count=1&language=en&format=json';
  const r=await fetch(u);
  if(!r.ok)throw new Error('geocoding HTTP '+r.status);
  const j=await r.json();
  const x=j.results?.[0];
  if(!x)throw new Error('city');
  const v={lat:x.latitude,lon:x.longitude,name:x.name||raw,timezone:x.timezone||'auto'};
  weatherCache[key]=v;
  saveWeatherCache();
  return v;
}
...
function formatTimezoneDiff(tz,dateKey){
  if(!tz||!dateKey||tz==='Asia/Tokyo'||tz==='Japan')return '';
  try{
    const [y,m,d]=dateKey.split('-').map(Number);
    const probe=new Date(Date.UTC(y,m-1,d,12,0,0));
    const diff=Math.round((timezoneOffsetMinutesAt(probe,tz)-timezoneOffsetMinutesAt(probe,'Asia/Tokyo'))/30)*30;
    if(diff===0)return '日本より ±0時間';
    const sign=diff>0?'＋':'−';
    const abs=Math.abs(diff);
    const h=Math.floor(abs/60), mins=abs%60;
    return `日本より${sign}${h}${mins?`時間${mins}分`:'時間'}`;
  }catch(e){return '';}
}
async function fetchWeather(city,date){
  ...
  if(weatherCache[cacheKey]){
    if(!weatherCache[cacheKey].timezone){
      const geo=await getWeatherCoords(rawCity);
      weatherCache[cacheKey].timezone=geo.timezone||'';
      saveWeatherCache();
    }
  ...
  let url,kind;
  if(rawDate<todayKey){
    url=`https://archive-api.open-meteo.com/v1/archive?latitude=${geo.lat}&longitude=${geo.lon}&start_date=${rawDate}&end_date=${rawDate}&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=${encodeURIComponent(geo.timezone||'auto')}`;
    kind='actual';
  }else{
    const diff=Math.floor((targetMs-todayMs)/86400000);
    // Open-Meteo's forecast API supports up to 16 forecast days, counting today.
    // Request only the required range so the requested date is explicitly included.
    if(diff>=16)return null;
    const forecastDays=Math.max(1,diff+1);
    url=`https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&daily=weather_code,temperature_2m_max,temperature_2m_min&forecast_days=${forecastDays}&timezone=${encodeURIComponent(geo.timezone||'auto')}`;
    kind='forecast';
  }
  const r=await fetch(url);
  if(!r.ok)throw new Error('weather HTTP '+r.status);
  const j=await r.json();
  const times=Array.isArray(j.daily?.time)?j.daily.time:[];
  const i=times.indexOf(rawDate);
  if(i<0)return null;
  const out={city:geo.name||rawCity,date:rawDate,kind,timezone:geo.timezone,code:j.daily.weather_code?.[i],max:j.daily.temperature_2m_max?.[i],min:j.daily.temperature_2m_min?.[i]};
  if(out.code==null&&out.max==null&&out.min==null)return null;
  weatherCache[cacheKey]=out;
  saveWeatherCache();
  return out;
}
...
async function renderWeather(t,dayIndex){
  ...
  const textEl=el.querySelector('.weather-text');
  const endIconEl=el.querySelector('.weather-end-icon');
  const timezoneEl=$('timezoneDiff');
  const city=(t.weatherCity||(/台湾|Taiwan/i.test(String(t.title||''))?'Taipei':/シンガポール|Singapore/i.test(String(t.title||''))?'Singapore':t.id==='seoul-2026'?'Seoul':'')).trim();
  const date=t.startDate?formatDateOnly(t.startDate,dayIndex):'';
  el.classList.remove('hidden');
  el.disabled=false;
  if(iconEl)iconEl.textContent='☀️';
  if(textEl)textEl.textContent='天気不明';
  if(endIconEl)endIconEl.textContent='☔️';
  if(timezoneEl)timezoneEl.textContent=formatTimezoneDiff(t?.weatherTimezone||'',date);
  if(!city||!date)return;
  if(textEl)textEl.textContent='読み込み中…';
  try{
    const w=await fetchWeather(city,date);
    if(!w)return;
    const [icon]=weatherLabel(w.code);
    if(timezoneEl)timezoneEl.textContent=formatTimezoneDiff(w.timezone,date);
    if(iconEl)iconEl.textContent=icon;
    const max=Number.isFinite(Number(w.max))?`${Math.round(Number(w.max))}℃`:'--℃';
    const min=Number.isFinite(Number(w.min))?`${Math.round(Number(w.min))}℃`:'--℃';
    if(textEl)textEl.textContent=`${w.city} ${max}/${min}`;
    if(endIconEl)endIconEl.textContent='';
  }catch(e){console.warn('[Tabiori weather]',e);}
}
function render(){
...
function openTripEdit(id){const t=tripList.find(x=>x.id===id);if(!t)return;editingTripId=id;editingTripDateMode=t.dateMode==='unset'?'unset':'set';$('tripEditTitle').value=t.title||'';$('tripEditFlag').value=t.flag||'';$('tripEditStart').value=t.startDate||'';$('tripEditEnd').value=t.endDate||'';$('tripEditHotel').value=t.hotel||'';applyTripDateEditMode();show('tripEditBackdrop');lock();}
...
function bind(){
 document.body.addEventListener('click',e=>{
   ...
   if(e.target.closest('#addTripButton')){const id=makeId();tripList.push({id,title:'新しい旅行',flag:'',startDate:null,endDate:null,dateMode:'set',weatherCity:'',hotel:''});ensureItems(id);saveAll();renderTripList();openTripEdit(id);return;}
