...
async function fetchWeather(city,date){
  ...
  const url=`https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&daily=weather_code,temperature_2m_max,temperature_2m_min&forecast_days=16&timezone=${encodeURIComponent(geo.timezone||'auto')}`;
  ...
  if(i<0)return null;
  const out={city:geo.name||rawCity,date:rawDate,kind:'forecast',code:j.daily.weather_code?.[i],max:j.daily.temperature_2m_max?.[i],min:j.daily.temperature_2m_min?.[i]};
