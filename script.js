...
const planDetails = {
  ...
  "オダリチプ": { place:"オダリチプ 明洞直営店", memo:"17:30予約済み。予約時間に遅れないよう注意。", route:"明洞ショッピング\n↓ 徒歩\nオダリチプ", query:"오다리집 명동" },
...
function openDetail(item) {
  const detail = planDetails[item.name];
  const title = document.getElementById("detailTitle");
  const content = document.getElementById("detailContent");
  const backdrop = document.getElementById("detailBackdrop");
  if (!title || !content || !backdrop) return;

  title.textContent = item.name || "予定の詳細";
  const place = detail ? detail.place : "詳細情報を準備中";
  const memo = detail ? detail.memo : (item.detail || "詳細情報を準備中");
  const route = detail ? detail.route : "この予定の移動情報は準備中です。";
  const mapButton = detail && detail.query
    ? `<a class="map-button" href="${naverMapSearchUrl(detail.query)}" target="_blank" rel="noopener">🗺️ NAVER Mapで開く</a>`
    : "";
  ...
  content.innerHTML = `
    <div class="detail-hero">
      <div class="icon-bubble">${item.icon || "📍"}</div>
      <div>
        <p class="detail-time">${item.time || ""}</p>
        <h3 class="detail-name">${item.name || ""}</h3>
      </div>
    </div>
    <section class="detail-section"><h3>📍 場所</h3><p>${place}</p></section>
    <section class="detail-section"><h3>🚇 行き方</h3><div class="route-box">${route}</div></section>
    <section class="detail-section"><h3>📝 メモ</h3><p>${memo}</p></section>
    ${mapButton}
  `;
...
function attachDetailHandlers() {
  document.querySelectorAll(".plan-card").forEach((card, index) => {
    ...
    card.addEventListener("click", function() {
      const day = trips[selectedDay];
      const item = day && day.items ? day.items[index] : null;
...
if (closeDetailButton) closeDetailButton.addEventListener("click", closeDetail);
...
const observerTarget = document.querySelector(".timeline") || document.body;
const detailObserver = new MutationObserver(function() {
  attachDetailHandlers();
});
detailObserver.observe(observerTarget, { childList:true, subtree:true });
attachDetailHandlers();
