...
saveItineraries();
let currentTripId=null,selectedDay=0,detailIndex=null,timeIndex=null,editingDays=null,editingTripId=null,longPressTriggered=false,addPlanIcon='➕️',detailEditIcon='';
...
function openDetail(i){const days=getItinerary(currentTripId);const x=days[selectedDay]?.items?.[i];if(!x)return;detailIndex=i;detailEditIcon=x.icon||'➕️';$('detailEditTitle').value=x.name||'';$('detailEditLocation').value=x.location||'';$('detailEditMemo').value=x.detail||'';document.querySelectorAll('[data-detail-icon]').forEach(b=>b.classList.toggle('is-selected',b.dataset.detailIcon===detailEditIcon));show('detailBackdrop');lock();}
...
function bind(){
 document.body.addEventListener('click',e=>{
   ...
   const sk=e.target.closest('[data-skip]');if(sk){editingDays[selectedDay].items[Number(sk.dataset.skip)].skipped=!editingDays[selectedDay].items[Number(sk.dataset.skip)].skipped;renderAdjust();return;}
