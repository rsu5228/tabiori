...
/* ===== v0.7.7 DAY navigation repair ===== */
(function(){
  function switchDay077(n){
    n=Number(n);
    if(!Number.isInteger(n)||n<0||n>2)return;
    if(typeof selectedDay!=="undefined") selectedDay=n;
    if(typeof render==="function") render();
  }
  function bindDay077(){
    const root=document.getElementById("tripDetailView")||document.body;
    if(root.dataset.day077)return;
    root.dataset.day077="1";
    root.addEventListener("click",function(e){
      const b=e.target.closest("[data-day-index]");
      if(!b||!root.contains(b))return;
      e.preventDefault(); e.stopImmediatePropagation();
      switchDay077(b.dataset.dayIndex);
    },true);
  }
  function tagDay077(){
    const root=document.getElementById("tripDetailView")||document.body;
    root.querySelectorAll("button,[role='button']").forEach(b=>{
      if(b.dataset.dayIndex!==undefined)return;
      const t=(b.textContent||"").replace(/\s+/g," ").trim();
      const m=t.match(/DAY\s*([1-3])|([1-3])\s*日目/i);
      if(m)b.dataset.dayIndex=String(Number(m[1]||m[2])-1);
    });
  }
  function init077(){bindDay077();tagDay077();}
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init077,{once:true});
  else init077();
})();

/* ===== v0.7.8 schedule detail repair ===== */
(function(){
  function openScheduleCard(card){
    const id=card.dataset.scheduleId || card.dataset.eventId || card.dataset.id;
    try{
      if(typeof openScheduleDetail==="function"){ openScheduleDetail(id, card); return true; }
      if(typeof showScheduleDetail==="function"){ showScheduleDetail(id, card); return true; }
      if(typeof openDetail==="function"){ openDetail(id, card); return true; }
      if(typeof showDetail==="function"){ showDetail(id, card); return true; }
      if(typeof openEventDetail==="function"){ openEventDetail(id, card); return true; }
    }catch(e){console.warn("schedule detail:",e);}
    return false;
  }
  function bindScheduleCards(){
    const root=document.getElementById("tripDetailView")||document.body;
    if(root.dataset.schedule078)return;
    root.dataset.schedule078="1";
    root.addEventListener("click",function(e){
      const card=e.target.closest(".schedule-card,[data-schedule-id],[data-event-id]");
      if(!card || !root.contains(card))return;
      // Ignore controls inside the card (edit/delete/time controls).
      if(e.target.closest("button,input,select,textarea,a,[data-no-card-click]"))return;
      openScheduleCard(card);
    },false);
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",bindScheduleCards,{once:true});
  else bindScheduleCards();
})();
