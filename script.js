...
function ensureItems(id){if(!Array.isArray(itineraries[id]))itineraries[id]=[{title:'DAY 1',items:[]},{title:'DAY 2',items:[]},{title:'DAY 3',items:[]}];itineraries[id]=normalize(itineraries[id]);}
...
function openTrip(id){
  ...
  currentTripId=id;
  getItinerary(id);
  if(!hasPlans(itineraries[id]) && id!=='seoul-2026') ensureItems(id);
  selectedDay=0;
  hide('tripListView');show('tripDetailView');render();window.scrollTo(0,0);
}
function render(){
  const t=tripList.find(x=>x.id===currentTripId);if(!t)return;
  $('tripFlag').textContent=t.flag||'✈️'; $('tripName').textContent=t.title||'旅行'; $('tripDate').textContent=formatDate(t.startDate,t.endDate); $('tripBadge').textContent=t.hotel||'';
  document.querySelectorAll('.day-tab').forEach((b,i)=>b.classList.toggle('active',i===selectedDay));
