...
function saveTimeEdit() {
  ...
  try {
    localStorage.setItem("tabioriTrips", JSON.stringify(trips));
  } catch (error) {}
...
function addTimeEditButtons() {
  const cards = document.querySelectorAll(".plan-card");
  cards.forEach((card, itemIndex) => {
    if (card.querySelector(".time-edit-button")) return;

    const button = document.createElement("button");
    button.type = "button";
    button.className = "time-edit-button";
    button.textContent = "時間変更";

    button.addEventListener("click", function(event) {
      event.preventDefault();
      event.stopPropagation();
      openTimeEdit(selectedDay, itemIndex);
    });
...
// Load saved schedules if available.
try {
  const savedTrips = localStorage.getItem("tabioriTrips");
  if (savedTrips) {
    const parsedTrips = JSON.parse(savedTrips);
    if (Array.isArray(parsedTrips) && parsedTrips.length) {
      trips.splice(0, trips.length, ...parsedTrips);
    }
  }
} catch (error) {}

const closeTimeEditButton = document.getElementById("closeTimeEdit");
if (closeTimeEditButton) closeTimeEditButton.onclick = closeTimeEdit;

const saveTimeEditButton = document.getElementById("saveTimeEdit");
if (saveTimeEditButton) saveTimeEditButton.onclick = saveTimeEdit;

const timeEditBackdrop = document.getElementById("timeEditBackdrop");
if (timeEditBackdrop) {
  timeEditBackdrop.addEventListener("click", function(event) {
    if (event.target === timeEditBackdrop) closeTimeEdit();
  });
}

// Ensure buttons appear after the schedule is rendered or re-rendered.
const timeEditObserver = new MutationObserver(function() {
  addTimeEditButtons();
});
timeEditObserver.observe(document.body, { childList: true, subtree: true });

addTimeEditButtons();


// ===== v0.5.1 Time edit modal fixes =====
(function(){
  function hideTimeEditModal(){const m=document.getElementById('timeEditBackdrop');if(m)m.classList.add('hidden');document.body.style.overflow='';}
  document.addEventListener('click',function(event){
    const closeButton=event.target.closest&&event.target.closest('#closeTimeEdit');
    if(closeButton){event.preventDefault();event.stopPropagation();hideTimeEditModal();}
  },true);
  document.addEventListener('click',function(event){
    const saveButton=event.target.closest&&event.target.closest('#saveTimeEdit');
    if(!saveButton)return;
    event.preventDefault();event.stopPropagation();
    const input=document.getElementById('timeEditInput');const newTime=input?input.value:'';if(!newTime)return;
    if(typeof editingDayIndex!=='undefined'&&typeof editingItemIndex!=='undefined'&&editingDayIndex!==null&&editingItemIndex!==null&&typeof trips!=='undefined'&&trips[editingDayIndex]&&trips[editingDayIndex].items&&trips[editingDayIndex].items[editingItemIndex]){
      trips[editingDayIndex].items[editingItemIndex].time=newTime;
      try{localStorage.setItem('tabioriTrips',JSON.stringify(trips));}catch(error){}
      hideTimeEditModal();
      if(typeof renderSchedule==='function'){renderSchedule();}
      else if(typeof renderPlans==='function'){renderPlans();}
...
// ===== v0.5.2 Tap time directly + keep current DAY =====
(function () {
  let tabioriEditingDay = null;
  let tabioriEditingItem = null;

  function removeTimeEditButtons() {
    document.querySelectorAll(".time-edit-button").forEach(btn => btn.remove());
  }

  function getTimeElement(card) {
    const candidates = [
      ".plan-time", ".time", ".schedule-time", ".card-time",
      "[data-role='time']"
    ];
    ...
    // Fallback: first element containing an HH:MM style time.
    return Array.from(card.querySelectorAll("*")).find(el =>
      el.children.length === 0 && /^\s*\d{1,2}:\d{2}/.test(el.textContent || "")
    ) || null;
  }

  function openDirectTimeEdit(dayIndex, itemIndex) {
    const day = trips[dayIndex];
    const item = day && day.items ? day.items[itemIndex] : null;
    if (!item) return;

    tabioriEditingDay = dayIndex;
    tabioriEditingItem = itemIndex;
  ...
  function makeTimesTappable() {
    removeTimeEditButtons();

    document.querySelectorAll(".plan-card").forEach((card, itemIndex) => {
      const timeEl = getTimeElement(card);
      if (!timeEl || timeEl.dataset.timeTapReady === "true") return;

      timeEl.dataset.timeTapReady = "true";
      timeEl.style.cursor = "pointer";
      timeEl.style.touchAction = "manipulation";

      timeEl.addEventListener("click", function (event) {
        event.preventDefault();
        event.stopPropagation();
        openDirectTimeEdit(selectedDay, itemIndex);
      });
    });
  }

  // Reliable save: update the same DAY currently being edited and render that DAY.
  document.addEventListener("click", function (event) {
    const saveButton = event.target.closest && event.target.closest("#saveTimeEdit");
    if (!saveButton || tabioriEditingDay === null || tabioriEditingItem === null) return;
    ...
    const day = trips[tabioriEditingDay];
    const item = day && day.items ? day.items[tabioriEditingItem] : null;
    if (!item) return;
    ...
    // Keep the DAY that was open before editing.
    selectedDay = tabioriEditingDay;
    ...
    tabioriEditingDay = null;
    tabioriEditingItem = null;

    setTimeout(makeTimesTappable, 50);
  }, true);

  // Remove old buttons and bind time elements after every render.
