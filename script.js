...
function openTimeEdit(dayIndex, itemIndex) {
  const item = trips[dayIndex] && trips[dayIndex].items
    ? trips[dayIndex].items[itemIndex]
    : null;
  if (!item) return;
...
function saveTimeEdit() {
  ...
  // Save using the app's original storage key so changes persist correctly.
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(trips));
  } catch (error) {}

  // Remember which DAY was open, then close and re-render that same DAY.
  const returnDay = editingDayIndex;
  const modal = document.getElementById("timeEditBackdrop");
  if (modal) modal.classList.add("hidden");
  document.body.style.overflow = "";
...
// Directly tap the left-side displayed time.
// Event delegation means it continues working after render().
document.addEventListener("click", function(event) {
  const timeBox = event.target.closest && event.target.closest(".timeline-item > .time");
  if (!timeBox) return;

  event.preventDefault();
  event.stopPropagation();

  const timelineItem = timeBox.closest(".timeline-item");
  const items = Array.from(document.querySelectorAll("#timeline .timeline-item"));
  const itemIndex = items.indexOf(timelineItem);
...
document.addEventListener("click", function(event) {
  ...
  if (backdrop && event.target === backdrop) closeTimeEdit();
});

// Save button.
const saveTimeEditButton = document.getElementById("saveTimeEdit");
if (saveTimeEditButton) {
  saveTimeEditButton.addEventListener("click", function(event) {
    event.preventDefault();
    event.stopPropagation();
    saveTimeEdit();
  });
...
// Remove any old "時間変更" buttons if present.
function removeOldTimeEditButtons() {
  document.querySelectorAll(".time-edit-button").forEach(button => button.remove());
}
removeOldTimeEditButtons();

new MutationObserver(removeOldTimeEditButtons).observe(document.body, {
  childList: true,
  subtree: true
});
