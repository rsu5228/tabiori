...
function currentTrip() {
  return tripList.find(t => t.id === currentTripId) || tripList[0];
}
...
function openTrip(id) {
  ...
  if (!trip) return;
  currentTripId = id;
  localStorage.setItem("tabiori_current_trip", id);
...
function renderTripList() {
  ...
  box.innerHTML = tripList.map(trip => `
    <button type="button" class="trip-card" data-trip-id="${trip.id}">
...
function addTrip() {
  const title = window.prompt("旅行名を入力してください", "新しい旅行");
  if (!title?.trim()) return;
  const destination = window.prompt("行き先を入力してください", "🇯🇵");
  if (destination === null) return;
  const date = window.prompt("日程を入力してください", "");
  if (date === null) return;
...
function bindTripManager() {
  document.getElementById("tripCards")?.addEventListener("click", e => {
    const card = e.target.closest(".trip-card");
    if (card) openTrip(card.dataset.tripId);
  });
  document.getElementById("addTripButton")?.addEventListener("click", addTrip);
  document.getElementById("backToTrips")?.addEventListener("click", showTripList);
}
