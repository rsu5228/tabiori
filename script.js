...
saveItineraries();
let currentTripId=null,selectedDay=0,detailIndex=null,timeIndex=null,editingDays=null,editingTripId=null,longPressTriggered=false,addPlanIcon='📍';
...
function openAdd(){ addPlanIcon='📍';$('addPlanName').value='';$('addPlanTime').value='';$('addPlanDetail').value='';document.querySelectorAll('[data-add-icon]').forEach(b=>b.classList.toggle('is-selected',b.dataset.addIcon===addPlanIcon));show('addPlanBackdrop');lock();}
...
function bind(){
 document.body.addEventListener('click',e=>{
   ...
   if(e.target.closest('#addPlanButton')){openAdd();return;}const iconChoice=e.target.closest('[data-add-icon]');if(iconChoice){addPlanIcon=iconChoice.dataset.addIcon||'📍';document.querySelectorAll('[data-add-icon]').forEach(b=>b.classList.toggle('is-selected',b===iconChoice));return;}if(e.target.closest('#closeAddPlan')||e.target.closest('#cancelAddPlan')){closeAllModals();return;}if(e.target.closest('#saveAddPlan')){if(!currentTripId)return;let days=getItinerary(currentTripId);if(!Array.isArray(days)||!days.length){ensureItems(currentTripId);days=getItinerary(currentTripId);}if(!days[selectedDay]){days[selectedDay]={title:`DAY ${selectedDay+1}`,items:[]};}if(!Array.isArray(days[selectedDay].items))days[selectedDay].items=[];const name=$('addPlanName').value.trim();if(!name)return alert('予定名を入力してください。');days[selectedDay].items.push({time:$('addPlanTime').value||'09:00',icon:addPlanIcon||'📍',name,detail:$('addPlanDetail').value||''});days[selectedDay].items.sort((a,b)=>toMin(a.time)-toMin(b.time));itineraries[currentTripId]=normalize(days);saveAll();closeAllModals();render();return;}
