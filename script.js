...
if (!Array.isArray(tripList) || !tripList.length) {
  tripList = [{id:"seoul-2026",title:"ソウル2泊3日",date:"9/5〜9/7",destination:"🇰🇷 ソウル",hotel:"相鉄フレッサイン ソウル明洞"}];
...
function openTrip(id){
  if(!tripList.some(t=>t.id===id)) return;
  currentTripId=id; localStorage.setItem("tabiori_current_trip",id); selectedDay=0;
...
function renderTripList(){
  ...
  box.innerHTML=tripList.map(t=>`<div class="trip-card-wrap" data-trip-id="${escapeHtml(t.id)}">
    <button type="button" class="trip-card" data-trip-id="${escapeHtml(t.id)}">
      <div class="trip-card-main"><div class="trip-card-title">${escapeHtml(t.destination||"")} ${escapeHtml(t.title||"")}</div>
      <div class="trip-card-date">${escapeHtml(t.date||"")}</div>${t.hotel?`<div class="trip-card-hotel">${escapeHtml(t.hotel)}</div>`:""}</div>
      <span class="trip-card-arrow">›</span>
    </button></div>`).join("");
}
function addTrip(){
  const title=prompt("旅行名を入力してください","新しい旅行"); if(!title?.trim())return;
  const destination=prompt("行き先を入力してください","🇯🇵"); if(destination===null)return;
  const date=prompt("日程を入力してください",""); if(date===null)return;
  const id="trip-"+Date.now(); tripList.push({id,title:title.trim(),destination:destination.trim(),date:date.trim(),hotel:""});
  localStorage.setItem("tabiori_trips",JSON.stringify(tripList)); renderTripList(); openTrip(id);
}
function editTrip(id){
  const t=tripList.find(x=>x.id===id); if(!t)return;
  const title=prompt("旅行名を編集",t.title||""); if(title===null)return;
  const destination=prompt("行き先を編集",t.destination||""); if(destination===null)return;
  const date=prompt("日程を編集",t.date||""); if(date===null)return;
  const hotel=prompt("ホテル・詳細を編集",t.hotel||""); if(hotel===null)return;
  t.title=title.trim()||t.title; t.destination=destination.trim(); t.date=date.trim(); t.hotel=hotel.trim();
  localStorage.setItem("tabiori_trips",JSON.stringify(tripList)); renderTripList();
}
function bindTripManagerV071(){
  const cards=document.getElementById("tripCards");
  if(cards&&!cards.dataset.bound071){
    cards.dataset.bound071="1"; let timer=null, suppress=false;
    cards.addEventListener("click",e=>{const c=e.target.closest(".trip-card");if(!c)return;if(suppress){suppress=false;return;}openTrip(c.dataset.tripId);});
    cards.addEventListener("pointerdown",e=>{const c=e.target.closest(".trip-card");if(!c)return;clearTimeout(timer);timer=setTimeout(()=>{suppress=true;editTrip(c.dataset.tripId);},650);});
    ["pointerup","pointercancel","pointerleave"].forEach(x=>cards.addEventListener(x,()=>clearTimeout(timer)));
    cards.addEventListener("contextmenu",e=>{const c=e.target.closest(".trip-card");if(!c)return;e.preventDefault();suppress=true;editTrip(c.dataset.tripId);});
  }
  const add=document.getElementById("addTripButton"); if(add&&!add.dataset.bound071){add.dataset.bound071="1";add.addEventListener("click",addTrip);}
  const back=document.getElementById("backToTrips"); if(back&&!back.dataset.bound071){back.dataset.bound071="1";back.addEventListener("click",showTripList);}
}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>{bindTripManagerV071();showTripList();},{once:true});
else{bindTripManagerV071();showTripList();}
