...
function openTrip(id){
  ...
  if(!hasPlans(days) && id!=='seoul-2026'){ensureItems(id);}
  selectedDay=0;
  hide('tripListView');show('tripDetailView');render();window.scrollTo(0,0);
}
function render(){
  const t=tripList.find(x=>x.id===currentTripId);if(!t)return;
  setText('tripFlag',t.flag||'✈️'); setText('tripName',t.title||'旅行'); setText('tripDate',formatDate(t.startDate,t.endDate)); setText('tripBadge',t.hotel||'');
