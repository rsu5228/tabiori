...
async function fetchWeather(city,date){const rawCity=(city||'').trim();const rawDate=(date||'').trim();if(!rawCity||!rawDate)return null;const cacheKey='weather:'+rawCity.toLowerCase()+':'+rawDate;if(weatherCache[cacheKey])return weatherCache[cacheKey];const geo=await getWeatherCoords(rawCity);const todayKey=localDateKey();const targetMs=Date.parse(rawDate+'T00:00:00');const todayMs=Date.parse(todayKey+'T00:00:00');if(!Number.isFinite(targetMs)||!Number.isFinite(todayMs))return null;let url,kind;if(rawDate<todayKey){url=`https://archive-api.open-meteo.com/v1/archive?latitude=${geo.lat}&longitude=${geo.lon}&start_date=${rawDate}&end_date=${rawDate}&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=${encodeURIComponent(geo.timezone)}`;kind='actual';}else{const diff=Math.round((targetMs-todayMs)/86400000);if(diff>16)return null;url=`https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&daily=weather_code,temperature_2m_max,temperature_2m_min&forecast_days=16&timezone=${encodeURIComponent(geo.timezone)}`;kind='forecast';}const r=await fetch(url);if(!r.ok)throw new Error('weather');const j=await r.json();const i=j.daily?.time?.indexOf(rawDate);if(i==null||i<0)return null;const out={city:geo.name||rawCity,date:rawDate,kind,code:j.daily.weather_code?.[i],max:j.daily.temperature_2m_max?.[i],min:j.daily.temperature_2m_min?.[i]};weatherCache[cacheKey]=out;saveWeatherCache();return out;}
