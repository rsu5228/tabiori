...
function openDetail(dayIndex, itemIndex) {
  const item = trips[dayIndex]?.items?.[itemIndex];
  if (!item) return;
...
function closeDetail() {
  hideModal("detailBackdrop");
  detailDayIndex = null;
  detailItemIndex = null;
}

function saveDetail() {
  if (detailDayIndex === null || detailItemIndex === null) return false;
  const item = trips[detailDayIndex]?.items?.[detailItemIndex];
  if (!item) return false;

  const title = document.getElementById("detailEditTitle")?.value.trim();
  const location = document.getElementById("detailEditLocation")?.value.trim();
  const memo = document.getElementById("detailEditMemo")?.value.trim();
  if (!title) {
    document.getElementById("detailEditTitle")?.focus();
    return false;
  }
  ...
  const returnDay = detailDayIndex;
  closeDetail();
  selectedDay = returnDay;
  render();
  return true;
...
function bindV061Events() {
  ...
  if (timeline) {
    timeline.addEventListener("click", (event) => {
      ...
      const card = event.target.closest(".timeline-item .plan-card");
      if (card && timeline.contains(card)) {
        openDetail(selectedDay, Number(card.dataset.detailIndex));
      }
    });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (!document.getElementById("detailBackdrop")?.classList.contains("hidden")) {
      closeDetail();
    }
...
function openDetail(dayIndex, itemIndex) {
  ...
  const el = getDetailElements();
  if (el.title) el.title.value = item.name || "";
  if (el.location) el.location.value = item.location || "";
  if (el.memo) el.memo.value = item.memo || item.detail || "";
...
function saveDetailPlan() {
  ...
  item.name = title;
  item.location = el.location?.value.trim() || "";
  item.memo = el.memo?.value.trim() || "";

  // Keep the card's existing detail text useful even if older data
  // only has "detail" instead of "memo".
  item.detail = item.memo || item.location || item.detail || "";
...
function deleteDetailPlan() {
  ...
  const returnDay = detailDayIndex;
  trips[returnDay].items.splice(detailItemIndex, 1);
  localStorage.setItem(STORAGE_KEY, JSON.stringify(trips));
...
function bindDetailEvents() {
  ...
  if (timeline) {
    timeline.addEventListener("click", (event) => {
      // Time remains the time-edit target.
      const timeEl = event.target.closest(".timeline-item > .time");
      if (timeEl) return;

      // Tapping the card (not the time column) opens detail.
      const card = event.target.closest(".timeline-item .plan-card");
      if (!card || !timeline.contains(card)) return;

      const row = card.closest(".timeline-item");
      const timeEl2 = row?.querySelector(".time");
      if (!timeEl2) return;

      const itemIndex = Number(timeEl2.dataset.timeIndex);
      openDetail(selectedDay, itemIndex);
    });
  }

  document.getElementById("closeDetail")?.addEventListener("click", closeDetail);
  document.getElementById("saveDetailPlan")?.addEventListener("click", saveDetailPlan);
  document.getElementById("deleteDetailPlan")?.addEventListener("click", deleteDetailPlan);
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", bindDetailEvents, { once: true });
} else {
  bindDetailEvents();
}
